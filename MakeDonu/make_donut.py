...
def DoPlot(gdf):
    ...
    if not g_outer.empty: g_outer.plot(ax=ax[0], color="none", 
                                       edgecolor="red",  linewidth=1.5)
    if not g_hole.empty:  g_hole.plot( ax=ax[0], color="none", 
                                      edgecolor="blue", linewidth=1.5)
    ax[0].set_title("outer (red) + hole (blue)")

    # 2nd view: outer_with_hole (pink)
    if not g_owh.empty: g_owh.plot(ax=ax[1], color="none", 
                                   edgecolor="pink", linewidth=1.5)
    ax[1].set_title("outer_with_hole (pink)")

    # 3rd view: outer_with_hole (pink) + MRR (black)
    if not g_owh.empty: g_owh.plot(ax=ax[2], color="none", 
                                   edgecolor="pink", linewidth=1.5)
    if not g_mrr.empty: g_mrr.plot(ax=ax[2], color="none",
                                   edgecolor="black", linewidth=1.5)
    ax[2].set_title("outer_with_hole (black) + MRR (green)")

    # 4th view: donut-1 (red), donut-2 (green)
    if not g_d1.empty: g_d1.plot(ax=ax[3], color="none", edgecolor="red",   linewidth=1.5)
    if not g_d2.empty: g_d2.plot(ax=ax[3], color="none", edgecolor="green", linewidth=1.5)
